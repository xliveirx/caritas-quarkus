...
import type { ParishRequest } from '@/shared/types/parish-request';
...
interface Props {
  open: boolean;
  onClose: () => void;
  onCreated: (parish: ParishResponse) => void;
}
...
export function ParishFormModal({ open, onClose, onCreated }: Props) {
  const { token } = useAuth();
  ...
  function validate(): boolean {
    ...
    if (rawDigits(form.cnpj).length !== 14) e.cnpj = 'CNPJ inválido (14 dígitos)';
    ...
    if (form.state.trim().length !== 2) e.state = 'Use a sigla do estado (ex: RS)';
    if (!form.postalCode.trim())   e.postalCode = 'CEP obrigatório';
    setErrors(e);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      const created = await api.post<ParishResponse>('/api/v1/parishes', payload, token);
      toast.success('Paróquia cadastrada!', `"${created.name}" foi adicionada com sucesso.`);
      onCreated(created);
      onClose();
    } catch (err) {
      ...
      toast.error(
        apiErr?.title ?? 'Erro ao cadastrar paróquia',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90dvh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200">
          <div>
            <h2 className="text-base font-bold text-slate-900">Nova Paróquia</h2>
            <p className="text-xs text-slate-500 mt-0.5">Preencha os dados da nova paróquia</p>
          </div>
          <button
            onClick={onClose}
            aria-label="Fechar"
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                <Field label="Nome da Paróquia" required error={errors.name}>
                  <input
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="col-span-2">
                <Field label="Logradouro" required error={errors.street}>
                  <input
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <Field label="CEP" required error={errors.postalCode}>
                <input
