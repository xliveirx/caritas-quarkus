...
function DoacoesTab() {
  ...
  async function handleCancel() {
    ...
    try {
      ...
      setEntries((prev) => prev.map((e) =>
        e.id === entryToCancel.id ? { ...e, status: 'CANCELED' as const } : e
      ));
  ...
  return (
    <>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                ...
                entries.map((entry) => (
                  <tr key={entry.id} onClick={() => setDrawerEntry(entry)}
                    ...
                    <td className="px-5 py-4" onClick={(e) => e.stopPropagation()}>
                      {entry.status === 'CONFIRMED' && (
                        <button type="button" aria-label="Cancelar entrada"
      ...
      <DonationEntryDrawer entry={drawerEntry} onClose={() => setDrawerEntry(null)} />
      <NewDonationEntryModal open={modalOpen} onClose={() => setModalOpen(false)} onCreated={handleCreated} />
...
export default function EntradaPage() {
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-slate-900">Entrada</h1>
        <p className="text-sm text-slate-500 mt-0.5">Registro de doações recebidas</p>
      </div>

      <div className="flex gap-1 p-1 rounded-xl bg-slate-100 w-fit mb-6">
        <button className="px-5 py-2 text-sm font-semibold rounded-lg bg-white text-slate-900 shadow-sm">
          Doações
        </button>
      </div>
