...
const CONDITION_COLORS: Record<string, string> = {
  ...
  USADO: 'bg-amber-50 text-amber-700 border-amber-100',
};
...
function ClothesTab() {
  ...
  const fetchItems = useCallback(async (currentPage: number) => {
    ...
    try {
      const data = await api.get<PaginatedResponse<ClothesStockItem>>(
        `/api/v1/stock/clothes?page=${currentPage}&size=${PAGE_SIZE}`,
        token
      );
      setItems(data.data);
  ...
  const headers = isAdmin
    ? ['Produto', 'Categoria', 'Tamanho', 'Gênero', 'Condição', 'Disponível', 'Paróquia']
                  ...
                  <td className="px-5 py-4">
                    {item.clothes.size
                      ? <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold
                          bg-slate-100 text-slate-700">{item.clothes.size}</span>
                      : <span className="text-slate-300">—</span>}
                  </td>
                  <td className="px-5 py-4 text-slate-600 text-sm">
                    {item.clothes.gender
                  ...
                  <td className="px-5 py-4">
                    {item.clothes.condition ? (
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full
                        ...
                        ${CONDITION_COLORS[item.clothes.condition] ?? 'bg-slate-100 text-slate-600 border-slate-200'}`}>
...
function FoodTab() {
  ...
  const fetchItems = useCallback(async (currentPage: number) => {
    ...
    try {
      const data = await api.get<PaginatedResponse<FoodStockItem>>(
        `/api/v1/stock/foods?page=${currentPage}&size=${PAGE_SIZE}`,
        token
      );
      setItems(data.data);
      setTotalItems(data.pagination.totalItems);
  ...
  const headers = isAdmin
    ? ['Produto', 'Lote', 'Validade', 'Disponível', 'Paróquia']
