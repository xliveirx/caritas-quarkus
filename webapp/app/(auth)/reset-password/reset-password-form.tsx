        ...
        <div className="w-full max-w-sm">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="px-8 py-10 flex flex-col items-center text-center gap-4">
              ...
              <div>
                <h1 className="text-base font-bold text-slate-900">Link inválido</h1>
                <p className="text-sm text-slate-500 mt-1 leading-relaxed">
                  Este link de redefinição não é válido. Solicite um novo pelo login.
                </p>
              </div>
              <Link
                href="/login"
        ...
        <div className="w-full max-w-sm">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="px-8 py-10 flex flex-col items-center text-center gap-4">
              ...
              <div>
                <h1 className="text-base font-bold text-slate-900">Senha redefinida com sucesso!</h1>
                <p className="text-sm text-slate-500 mt-1 leading-relaxed">
                  Sua nova senha foi configurada. Você já pode acessar a plataforma.
                </p>
              </div>
              <Link
                href="/login"
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="px-8 py-10 flex flex-col items-center text-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1 leading-relaxed">
                  Este link de redefinição expirou ou não é mais válido.
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.status === 400) {
        ...
        if (err?.message?.toLowerCase().includes("don't match") ||
            err?.message?.toLowerCase().includes("mismatch")) {
          setErrors((prev) => ({ ...prev, confirmPassword: 'As senhas não coincidem' }));
        } else {
          setApiError(err?.message ?? 'Não foi possível redefinir a senha. Tente novamente.');
        }
        return;
      }
      if (!res.ok) {
